...
def main():
    ...
    try:
        ...
        def one(i):
            t0 = time.time()
            st, body = _post(base + "/public/providers", {
            ...
            return {"i": i, "status": st, "carrier_id": d.get("carrier_id"),
                    ...
                    "err": d.get("error"), "ms": (time.time() - t0) * 1000.0}
        ...
        # concurrent probes still healthy
        hp = 0
        ...
        if fails:
            for r in fails[:5]:
                print(f"  FAIL client {r['i']}: status={r['status']} err={r['err']}")
        ok = (len(fails) == 0 and distinct == len(oks) == n and hp == 10)
        print("CONCURRENCY RESULT:", "PASS - simultaneous transactions handled with correct isolation"
              if ok else "FAIL")
