...
| C-004 | Organization hierarchy (company/BU/branch/dept/team/cost-center/site/warehouse/calendars) | 1 | P1 | C-003 | P0 | **✅ DELIVERED (25 tests)** | tenant-scoped org graph (adjacency-list `org_units`); circular/cross-tenant/inactive-parent/dup-code blocked; user assignments (primary/secondary/temporary); org-scoped authorization (branch/cost-center/site boundaries); holiday+working calendars w/ inheritance; company profile; config cascade extended to org levels; audit; PG-portable; tenant-backfill matrix produced; **Administration console API delivered** (36 /admin/* endpoints across Organization/People&Access/Calendars/Security/Configuration/Governance, permission-gated, +8 HTTP tests); **166-test suite green**. Admin console front-end (HTML) = next |
